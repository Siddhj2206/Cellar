...
impl ProtonManager {
    ...
    fn extract_version_from_name(&self, name: &str) -> String {
        ...
        if let Some(captures) = regex::Regex::new(r"(?i)proton[^\d]*(\d+(?:[.-]\d+)*)").unwrap().captures(name) {
    ...
    pub async fn download_ge_proton(&self, version: &str) -> Result<PathBuf> {
        ...
        let url = format!("https://api.github.com/repos/GloriousEggroll/proton-ge-custom/releases/tags/{version}");
        let response = client.get(&url).send().await?;
        
        if !response.status().is_success() {
            return Err(anyhow!("Failed to fetch release info for version {}", version));
        }
        ...
        let asset = release.assets.iter()
            ...
            .ok_or_else(|| anyhow!("No tar.xz asset found for version {}", version))?;
...
impl RunnerManager for ProtonManager {
    ...
    async fn get_available_versions(&self) -> Result<Vec<String>> {
        ...
        let response = client.get(url).send().await?;
        if !response.status().is_success() {
            return Err(anyhow!("Failed to fetch available versions"));
        }
