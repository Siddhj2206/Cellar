...
use crate::config::game::{GameConfig, GameInfo, LaunchConfig, WineConfig, GamescopeConfig, MangohudConfig, DesktopConfig, DependenciesConfig};
...
pub fn add_game(name: String, exe: Option<String>, installer: Option<String>, interactive: bool) -> Result<()> {
    ...
    let exe_path = exe.ok_or_else(|| anyhow!("Executable path is required for basic game addition"))?;
    ...
    println!("  Config saved to: {}", dirs.get_game_config_path(&name).display());
...
pub fn show_game_info(name: String) -> Result<()> {
    ...
    if config.gamescope.enabled {
        ...
        println!("  Resolution: {}x{}", config.gamescope.width, config.gamescope.height);
...
fn create_basic_game_config(name: &str, exe_path: PathBuf, dirs: &CellarDirectories) -> Result<GameConfig> {
