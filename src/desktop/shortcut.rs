...
pub async fn sync_desktop_shortcuts() -> Result<()> {
                    ...
                    Ok(config) => {
                        if config.desktop.create_shortcut {
                            match create_desktop_shortcut(&config, &game_config_name).await {
                                Ok(()) => created_count += 1,
                                Err(e) => {
                                    eprintln!("Failed to create shortcut for {}: {}", game_config_name, e);
                                }
                            }
                        } else {
                            skipped_count += 1;
    ...
    println!("Sync complete: {} shortcuts created, {} skipped", created_count, skipped_count);
    Ok(())
}
