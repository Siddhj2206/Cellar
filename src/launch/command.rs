...
impl CommandBuilder {
    ...
    fn build_base_command(&self) -> Result<Vec<String>> {
        let _proton_path = self.proton_path.as_ref()
    ...
    fn build_dxvk_environment(&self) -> Result<HashMap<String, String>> {
        ...
        if self.config.wine_config.dxvk {
            ...
            env.insert("DXVK_STATE_CACHE_PATH".to_string(), 
    ...
    fn build_mangohud_environment(&self) -> Result<HashMap<String, String>> {
        ...
        if self.config.mangohud.enabled {
            ...
            if let Some(config_file) = &self.config.mangohud.config_file {
                env.insert("MANGOHUD_CONFIGFILE".to_string(), 
    ...
    /// Process Steam-style launch options with %command% placeholder
    fn process_launch_options(&self, base_command: Vec<String>, _env_vars: &HashMap<String, String>) -> Result<ProcessedCommand> {
    ...
    fn wrap_with_gamescope(&self, command: Vec<String>) -> Result<ProcessedCommand> {
        ...
        match gamescope_config.upscaling.as_str() {
            "fsr" => gamescope_cmd.push("-U".to_string()),
            "nis" => gamescope_cmd.push("-Y".to_string()),
            "linear" => gamescope_cmd.push("-n".to_string()),
            "nearest" => gamescope_cmd.push("-b".to_string()),
            "off" => {}, // No upscaling flag
            _ => return Err(anyhow!("Invalid upscaling method: {}", gamescope_config.upscaling)),
        }
        ...
        if gamescope_config.borderless {
            gamescope_cmd.push("-b".to_string());
        }
        
        if gamescope_config.steam_integration {
            gamescope_cmd.push("-e".to_string());
        }
